    ...
    public class RateOrderItemCommandHandler : IRequestHandler<RateOrderItemCommand, RateOrderItemResultDto>
    {
        ...
        public async Task<RateOrderItemResultDto> Handle(RateOrderItemCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var orderItem = await GetOrderItemAsync(request.OrderItemId, cancellationToken);
                if (orderItem == null)
                ...
                orderItem.AddRating(request.Rating, request.Comment);
        ...
        private async Task<OrderItem?> GetOrderItemAsync(int orderItemId, CancellationToken cancellationToken)
        {
